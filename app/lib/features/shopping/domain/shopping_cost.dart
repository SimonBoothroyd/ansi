...
/// unit through [quantityInBasis], times the price the row's cost reads per
/// unit of that basis (ADR-0017). A row that cannot be priced adds nothing and
/// says `no price yet`; a trip with one reads `at least …` rather than `≈ …`,
/// and a trip nothing can price has no figure. The caller leaves ticked rows
/// out.
library;
...
/// What [item] comes to at the price its row's cost reads, in cents, or null
/// when it cannot be priced: no price, a total that never reached the basis
/// unit, a price against a different basis, or a free-text item. Every subtotal
/// must convert; pricing half a row would understate it.
