...
/// writes and reads.
 List<String> get steps;/// Tokenized method, rendered by [foldMethod]. The `steps` jsonb holds
/// either this or plain-text [steps], never both.
 List<MethodStep>? get methodSteps;/// Fridge shelf life; drives the cook-plan clustering, set from
/// the recipe editor's shelf-life inputs.
 int? get keepsForDays; bool get freezable; int? get freezerDays;/// The book and section this recipe is filed under; [sectionId] is null
/// when unsectioned. [bookName] / [sectionName] are denormalised on read
/// for display.
 String? get bookId; String? get sectionId; String? get bookName; String? get sectionName;/// Per-serving macros for the recipe as written, or an incomplete marker.
...
/// a [Recipe] without line nutrition.
 RecipeMacroSummary? get macros;/// What one batch makes ("makes 1 cup"). Independent of [servingsBase];
/// both halves are set or neither (a DB check). Null means a component
/// line's share of this recipe cannot be computed.
 double? get yieldQty; Unit? get yieldUnit;/// The optional second denomination of the same batch ("makes 250 g · 16
/// tbsp"), pinned by the database to a different unit family, so the pair
/// bridges mass↔volume for this recipe without a density.
 double? get yieldQty2; Unit? get yieldUnit2;/// The household's own words for a portion of this batch, in `sort_order`,
/// duplicates merged. A measure whose family the yields no longer state
/// stops resolving.
 List<RecipeMeasure> get measures;/// The printed cook and total times, in seconds. No rule relates them; null
/// is unset.
...
/// chips and the "same batch" hint without the full recipe.
 ...
 bool get favorite;/// Honest per-serving macros, or an incomplete marker.
 RecipeMacroSummary? get macros;/// What one batch makes, so a picker row can say "makes 1 cup" without
/// loading the recipe. See [Recipe.yieldQty].
 double? get yieldQty; Unit? get yieldUnit; double? get yieldQty2; Unit? get yieldUnit2;/// This recipe's own measures, `sort_order` first, merge-hidden twins
...
mixin _$LineItem {
 ...
 String get id; String get ingredientName;/// The catalog unit the [quantity] is said in, or null on a component line
/// said in one of the target's words ([recipeMeasureId]). Exactly one of
/// the two is set, as the database checks.
 Unit? get unit; String? get ingredientId; String? get subRecipeId; SubRecipeTarget? get subRecipe; double? get quantity; String? get measureId; Measure? get measure;/// The target recipe's own word this line is said in (`3 blob`). Set only
...
/// [SubRecipeTarget.measures], not joined onto the line.
 String? get recipeMeasureId; String? get note; bool get optional;/// Whether [measureId] points at a measure the household deleted, as
/// opposed to one that has not synced yet. Both leave [measure] null.
 bool get measureDeleted;/// Whether [ingredientId] points at a retired vocab row (`deleted_at` set).
...
mixin _$SubRecipeTarget {
 ...
 String get id; String get title; double? get yieldQty; Unit? get yieldUnit; double? get yieldQty2; Unit? get yieldUnit2;/// The target's live measures, `sort_order` first, including merge-hidden
