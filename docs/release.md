...
>   33758932599, same three jobs green, APK 92 MB + AAB 80 MB on the Release) and `v0.5.0` (2026-09-04, the state-of-the-world sweep — plan 0030, run 33903706382, APK 93 MB + AAB 80 MB on the Release, after the 0026–0031 cloud push) and `v0.5.1` (2026-09-04, the import review fixes from the wild-garlic hunt — run 33912875394, APK 93 MB + AAB 80 MB, no migrations so no cloud step) and `v0.6.0` (2026-09-05, field test round five — seven plans built in parallel lanes; run 33961427290, APK 88 MB + AAB 76 MB on the Release, `play-internal` skipped by design; after the 0032–0034 cloud push, run 33961320740) and `v0.7.0` (2026-09-08, field test round six — the four owner notes, the all-to-all unit rule and the vocabulary unit audit; run 34237807444, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; shipped onto a cloud database **rebuilt from scratch** — owner call, cloud-setup §2c — then deploy-supabase run 34237512787) and `v0.8.0` (2026-09-08, the piece weight — ADR-0015, plan 0042; run 34275484036, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; after the 0039 cloud push with the template reseed ticked, deploy-supabase run 34274932829) and `v0.9.0` (2026-09-08, four owner notes built in parallel lanes — a camera door on the import form, an ingredient's name on a recipe line opens its page, a chip's new word keeps the sentence's case, and a ⋯ toggle prints each line's macros; run 34284901170, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no migrations so no cloud step) and `v0.10.0` (2026-09-08, the ingredient page reads before it edits — a fact sheet with Edit behind ⋯ — every default-unit chip live with the stranded refusal as the one gate, one density note instead of two, and typed names tidied when a field is left (Title Case for ingredients, a told suggestion with keep-the-old-word); run 34300128198, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no migrations so no cloud step) and `v0.11.0` (2026-09-09, five owner notes on v0.10.0 — chips take the sentence's case across every word and every name kind is Title Case, the default-unit row shows only what the row can say with one note for the rest, a new ingredient saves complete or not at all, a beverage's barcode label lands per 100 ml, and a photo import waits as long as the function can run behind a stage ladder; the two smoke files red on main are green again; run 34411719268, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; edge function redeployed first — deploy-supabase run 34409373514, no migrations) and `v0.11.1` (2026-09-09, a UPC-A typed as the pack prints it — ten middle digits — is completed with its number-system and check digits instead of refused; run 34413971735, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no cloud step) and `v0.11.2` (2026-09-09, a pack quantity with a stray comma and a label served by the cup no longer read as per 100 ml; run 34414628238, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no cloud step) and `v0.12.0` (2026-09-09, the serving redrawn — the per-serving row loses its free-text field and takes any kitchen unit, the density sentence takes an amount and is the one place a density is stated, the serving is kept as a measure so the page prints the label's figures first, a scan seeds that serving from the pack's bracket, and fibre is the optional fifth macro — a total states it only when every line did; run 34423455276, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no cloud step — the seed's fibre reaches households on the next template reseed + rollout) and `v0.12.1` (2026-09-09, the macro fields as one sentence of inline slots, a scan with a printed serving landing in per-serving mode with the pack's figures, a nudge on a scanned per-100 row that names no serving, kcal whole and grams to one decimal wherever printed, and flame and wheat glyphs in the dense macro lines; run 34431917413, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no cloud step) and `v0.12.2` (2026-09-09, the serving row at the same inline height as the figures, its unit picker sized to its word, and a gap between the two; run 34433442728, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no cloud step) and `v0.12.3` (2026-09-10, the unit picker at the inline height, a stated density staying folded, and a scan naming a row whose only stamp was a borrowed density; run 34470260590, APK 93 MB + AAB 80 MB on the Release, `play-internal` skipped by design; no cloud step) and `v0.13.0` (2026-09-11, field test round seven — eight parallel lanes: kitchen fractions, one amount-and-unit control, measures renamed and reordered in place, one name namespace with a did-you-mean, the import cascade batched and its reading screen streamed from the server, the shop counting a row in the measure it was asked for, the seed as the owner's own snapshot, and a recipe varied for one week; run 34561874981, APK 94 MB + AAB 81 MB on the Release, `play-internal` skipped by design; after the 0040 cloud push with the template reseed ticked, deploy-supabase run 34561587312 — the function and the app share one wire shape, so they shipped together) and `v0.13.1` (2026-09-11, field test round eight on v0.13.0 — the unit half of every sentence is the quantity sheet's own chip at one inline height, the ingredient form reopens in the mode the numbers were entered in with hints that say what to do, the USDA door drawn where it is needed, adding from a day asks only the slot, "Edit for this week" on Cook and Plan only, the import function streaming its model calls behind heartbeats, import's learned aliases held to the one namespace, and the allowed-units ladder applied once to the owner's vocabulary as a reviewed data pass; run 34658602960, APK 94 MB + AAB 81 MB on the Release, `play-internal` skipped by design; edge function and the 315-row template reseed first — deploy-supabase run 34658424680, no migrations) and `v0.13.2` (2026-09-11, field test round nine on v0.13.1 — the import reading marker spinning in place and its checklist in the tense it is in, an optional line wearing its tag on the editor row and that tag being the switch in week mode, and import's learning loop refusing a whole printed line as a name; run 34667784744, APK 94 MB + AAB 81 MB on the Release, `play-internal` skipped by design; edge function and the template reseed first, after eight learned aliases were retired on the owner's household — deploy-supabase run 34667684551, no migrations).
