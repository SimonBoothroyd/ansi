...
Parsed from `supabase/migrations/*.sql` (38 migrations, 19 tables). Per table: columns from `create table` plus later `alter table add column`s, whether RLS is enabled, whether the table is in the `powersync` publication, and the migration that introduced it.
