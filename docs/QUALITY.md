...
| Ingredient data model + vocab | 🟢 | Migrations `0001`/`0002` + RLS, 291-ingredient vocab, macros prefilled server-side. `ingredient_measure` (`0009`+`0010`): 183 GENERATED starter measures over 118 ingredients (USDA-FDC piece portions via `gen_measures.ts`, per-row `source` provenance incl. explicit + auto-detected borrows and a basis-qualifier guard; 7 `seed:typical` survivors), synced + cloned at onboarding, **backfilled** into measure-less households on next sign-in (0010), watched per-ingredient repo. Density still sparse; no in-app measure editor yet (tracker — lands in 7.7). |
