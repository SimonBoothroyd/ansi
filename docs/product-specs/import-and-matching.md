...
- **`ingredient`** — the household's curated, lean vocabulary (the seeded template alone is 319 rows). This is the match target and the only ingredient data that syncs to devices. Seeded small; grows through the one add flow — the **flesh-out form** at `/ingredients/new`, wherever it is opened, the import review included (§9).
