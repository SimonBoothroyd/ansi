...
| — | Field test, round eight on `v0.13.0` — the amount-and-unit control is the quantity sheet's own unit chip at one inline height in every host; the ingredient form reopens per serving when the row states one and the density sentence opens where the fact sheet reads, with hints that say what to do; the USDA door is drawn where it is needed; adding from a day asks only the slot; "Edit for this week" lives where a planned recipe is looked at (Cook and Plan, not the Library); the import function streams its model calls and heartbeats so a long reading never trips the client's silence budget; import's learned aliases run the one-namespace collision check; and the allowed-units ladder was applied once to the owner's vocabulary as a reviewed data pass, never as an app rule, with the 315-row snapshot promoted as the template. No migrations | 🟢 `v0.13.1` | — |
| — | Field test, round nine on `v0.13.1` — the import reading marker spins in place and the checklist speaks in the tense it is in; an optional line wears its tag on the editor row, and in week mode that tag is the switch that keeps the line for the week (an `include` row, the shop and week macros already honouring it); import's learning loop refuses a whole printed line (comma, or, slash, bracket) as a name, and the eight learned lines were retired on the owner's household with the seed regenerated at 145 aliases. No migrations | 🟢 `v0.13.2` | — |
| — | The retire guard — an ingredient is retired only when nothing live names it: the database refuses the retire while a recipe line, planned meal or this-week swap names the row and repairs lines an earlier pass left broken; a line at a retired row is shown by its last name and repairable on the page, in the editor, in the shop's echo and on the import review. Migration `0041` | 🟡 built, holding for the owner's own changes; ships as `v0.13.3` | — |
| — | Round ten on `v0.13.2` — the recipe page up-levelled: a method chip is the word with its live amount in a pill, step numbers are digits, the Method tab says what it is scaled to, the planned band is one line, amounts print as kitchen-fraction glyphs (`1½`, held by a font test), a line's macros read in the panel's order with whole grams, and the panel names what it left out in two labelled rows. And optional is a decision the cook can reach: the page opened from a week draws the week's lines and its `optional` tag is the switch, the shop's echo names are doors, and the cook plan reads the component graph through the same seam so an optional sub-recipe is cooked only when included. No migrations | 🟡 in progress — on main, not yet released | [0044](./active/0044-recipe-page-and-optional.md) |
