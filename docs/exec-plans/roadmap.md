...
| — | Food cost, phase one — a price is a row fact with history (cents for a stated pack, in the words it was bought in, at a store, on a date; a hand-typed price is a one-line receipt) with an edit and delete door on every row, and a recipe, the week band and the shop read what their lines cost at the latest price: the recipe panel flips between Macros and Cost, one `Show line figures` toggle follows it, a planned snack costs what its row costs, and an unpriced line is named, never zeroed. ADR-0017. Migrations 0044, 0046 | `v0.19.0` | [0049](./active/0049-food-cost-receipts-and-meals-out.md) (phase one) · [ADR-0017](../decisions/0017-a-cost-is-a-unit-price-never-an-allocation.md) |
| — | A meal eaten out — a third kind of planned meal: its words, its eaters and the per-portion macros when they were stated. It fills its slot, is neither cooked nor bought, and counts in the day only as stated; every derivation branches on the kind, held by a seam test. Also: the picker says what a missed search missed, and every type role names its face. Migration 0045 | `v0.19.0` | [0049](./active/0049-food-cost-receipts-and-meals-out.md) (phase three) |
| — | Food cost, phase two — a photographed receipt reads through the recipe import's own pipeline and is confirmed line by line: money first, the store as a chip word over the paper's header, the receipt's own date, and the lines' sum held against the printed subtotal as a flag rather than a refusal. A by-weight line prices itself; one with no printed weight opens on the pack that row was last bought in, and *keep as a measure* is the one tap that mints a word. No alias is learned. Save writes one receipt and every line it kept; the ledger files by the household's week and store, and the week band gains its second figure. Taking a price back on a photographed line clears its price facts and keeps the line. No migrations | `v0.20.0` | [0049](./active/0049-food-cost-receipts-and-meals-out.md) (phase two) |
| — | Food cost, the first feedback pass — the owner's own 29-line strip read back: a dashed date parses, a paper with no printed subtotal holds its lines against the total less tax and says so on the join card, an unmatched card is titled by the printed name alone, Bought is a calendar door, PRICE is a chip on every open card, and one screen serves a saved receipt — the review on its rows, Save rewriting them in place, delete under it. The twins of a line on the same receipt take its answer with it. The household's own saved lines are the match memory, recalled by printed name and taught to no alias, and the pack pre-fills under those same words rather than the row. *Keep as a measure* names the word it buys and the household's own style for writing one, with one authoring rule wherever a label is typed and a word that states its size stating it once. The unit picker drops the serving, opens on its first chip and leads with the imprecise default; a measure that lands leaves the form ready for the next, keeping the unit it was weighed in; the ingredient editor carries the Price group; and any aggregate that left a whole meal out says `at least`. Migration `0047` | `v0.21.0` | [0049](./active/0049-food-cost-receipts-and-meals-out.md) (feedback pass) |
| — | A recipe's own word for a share of what it makes, the **reader** half — a recipe coins a named amount (`blob` = 15 g), each measure carrying its own unit and refused while the recipe states no `makes` in that unit's family; a component line is denominated in a unit *or* in one of those words, and a measured line is read, costed, cooked and shopped in the word it was written in, never silently re-denominated. The component chip row leads with the recipe's own words, and the data layer says what a word is rather than how many fit. No authoring door: a build older than `v0.22.0` fails on a component line whose `unit` is null, so the readers shipped a release ahead. Migration `0048`, on cloud with its sync stream | `v0.22.0` | [ADR-0018](../decisions/0018-a-recipe-measure-is-a-named-amount.md) |
| — | A learned alias names a thing, not a decision — a decision word is matched with a plain pattern and the import's learning loop refuses it, held by a seed guard that allows no decision-shaped row; and the seed is the owner's cleaned household again, its packaging labels in the house style `pack (N oz)` (323 ingredients · 326 measures · 157 aliases). Reseed on deploy | `v0.22.0` | — |
| — | A recipe's own measures, the **authoring** half — the editor's MEASURES group sits under what the recipe makes, and the component amount sheet picks a measure or coins one behind its `+` wherever a component's amount is edited. Every phone in the household must already be on `v0.22.0` or later. No migrations | `v0.23.0` | [ADR-0018](../decisions/0018-a-recipe-measure-is-a-named-amount.md) |
| — | The review's fixes — a recipe measure's unit must be in a measuring family, a coined measure returns to the amount, a measured line resolves its word by id; a receipt edit tombstones only the lines it dropped and a failed write keeps the review. Migration `0049` | `v0.23.1` | — |
| — | A receipt line carries a count — a count sub-row belongs to the line above it, and a price is paid ÷ (count × pack). Migration `0050` | `v0.24.0` | [0049](./active/0049-food-cost-receipts-and-meals-out.md) |
| — | A receipt line the reader missed is added by hand; it stores no printed words, so the match memory learns nothing from it. No migrations | `v0.24.1` | [0049](./active/0049-food-cost-receipts-and-meals-out.md) |
