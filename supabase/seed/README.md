...
2. **Initial household `ingredient` vocabulary** — mined from real recipes and
   curated (currently 311 rows), so the app isn't empty on first run. Macros +
   density are prefilled from `usda_food` where a true match exists, plus a
   handful of label-sourced macro fills in the curation pass (275
...
Current vocab coverage: **300/311** (264 → 277 when the FAO fallback landed,
