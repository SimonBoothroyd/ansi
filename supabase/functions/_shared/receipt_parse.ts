...
// -----------------------------------------------------------------------------
// The words that name the thing, read off the printed line
// -----------------------------------------------------------------------------

const MONEY = /-?\$?\d+\.\d{2}(?!\d)/g;
const WEIGHT_AT_RATE =
  /(?:qty\s*)?\d+(?:\.\d+)?\s*(?:lbs?|kg|oz|g)\.?\s*@\s*\$?\d+\.\d{2}\s*\/\s*(?:lbs?|kg|oz|g)\b/gi;
const COUNT_AT_EACH = /(?:qty\s*)?\d+\s*@\s*\$?\d+\.\d{2}(?:\s*ea\b)?/gi;
const QTY_EACH = /qty\s*\d+\s*\$?\d+\.\d{2}\s*ea\b/gi;
const TAX_FLAG = /\s+(?:F|T|FT|TF|N|X)\s*$/;

/**
 * A printed line with its figures taken off: the weight-and-rate, the count
 * sub-row, the money, and the tax flag a till prints after the money.
 */
export function nameFromPrintedLine(printed: string): string {
  const hadMoney = printed.match(MONEY) !== null;
  let s = printed
    .replace(WEIGHT_AT_RATE, " ")
    .replace(QTY_EACH, " ")
    .replace(COUNT_AT_EACH, " ")
    .replace(MONEY, " ");
  if (hadMoney) s = s.replace(TAX_FLAG, " ");
  return s.replace(/\s+/g, " ").trim();
}

/**
 * The model's name, unless it is a shortened form of what the line itself says
 * — `ORG` for `ORG TAHINI $4.79` — in which case the line's own words win.
 */
export function repairedName(modelName: string, printed: string): string {
  const model = modelName.replace(/\s+/g, " ").trim();
  const derived = nameFromPrintedLine(printed);
  if (derived === "" || derived.length <= model.length) return model;
  return derived.toLowerCase().startsWith(model.toLowerCase())
    ? derived
    : model;
}
