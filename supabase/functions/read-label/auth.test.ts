// The label door's binding of the shared gate. The ladder is covered in
...
Deno.test("label auth — ONE allowlist, shared with the two import doors", () => {
  // Not `LABEL_ALLOWED_HOUSEHOLDS`: one allowlist covers all three.
...
Deno.test("label auth — a household off the allowlist is 403, in this door's words", () => {
  withAllowlist(OTHER, () => {
    const r = readCaller(request(`Bearer ${bearer({ household_id: HH })}`));
    assert("error" in r);
    assertEquals(r.status, 403);
    assertEquals(r.error, "reading a label is not enabled for this household");
    // Never echoes the household id back to an unauthorised caller.
    assert(!r.error.includes(HH));
  });
});

Deno.test("label auth — no household claim is 403, no token is 401", () => {
