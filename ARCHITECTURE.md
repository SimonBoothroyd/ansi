...
| Wide screens (`shared/ansi_layout.dart`) | 🟡 | One file reads the viewport — three bands off Forui's own breakpoints, and a 640 measure applied in exactly two places (the tab shell wraps what it owns, the router's page helper wraps every pushed page), held by a structural test that fails on a `MediaQuery.sizeOf`, a `.size` or a `LayoutBuilder` anywhere else under `lib/`. The first views to use the room rather than be centred in it are the Library's shelf of book tiles and the book page's two panes, which opts out of the measure through the page helper's own flag. Still absent: chrome beside the content, sheets that become dialogs, and the wide answers drawn for Week, the recipe page and the vocabulary. | [wide-screen.md](./docs/design-docs/wide-screen.md) |
