...
| Receipts (`features/receipts`) | 🟢 | A photographed receipt reads through the recipe import's own pipeline — the same camera, the same crop, the same checklist over the same event stream, with four stages named for what they read — and is confirmed line by line: money first, the store as a chip word over the paper's own header, the receipt's own date, and the lines' sum held against the printed subtotal as a **flag rather than a refusal**. Two questions a recipe line never asks: what the pack is (the price sheet's own field, with the household's tap the one thing in the app that mints a measure from an import) and what the line is, with *Not food* folding it under the list where it counts toward the trip and toward no price. A line sold by weight prices itself from the printed weight, and one with none opens on the pack that row was last bought in — **the pack carries over, never a word**, because a receipt's text is one store's abbreviations and no alias is learned from it. Save writes the receipt and every line it kept in one transaction, and nothing before it. The ledger files by the household's week and by store, spent beside planned, and a saved receipt reads back through the same card, read-only. The desk gets the phone's column at the measure; its three-column form is drawn and unbuilt. | [import-and-matching.md](./docs/product-specs/import-and-matching.md) · [ADR-0017](./docs/decisions/0017-a-cost-is-a-unit-price-never-an-allocation.md) |
