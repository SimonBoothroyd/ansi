...
| Planning (`features/planning`) | 🟢 | Week v3's three drawn targets, the viewed week that Cook and Shop follow, and fractional demand from each member's `portion_factor`; a slot now takes a recipe **or** an ingredient, one target enforced by a server check, and a snack carries eaters and multiplies like any other entry. A recipe can be varied for one week — one variant per (week, recipe), so every day that plans it shares one pot — reached through one door at the foot of the meal editor rather than a control every dish row pays for, marked `edited for this week` where it is planned, and named by copy-last-week among the things it could not bring. The household picks which day its week starts on: a week is addressed by the date of its own first day and a meal's `day_of_week` is the offset from it, so the shop done on a Sunday covers that Sunday's dinner. | [product-spec.md](./docs/product-specs/product-spec.md) |
