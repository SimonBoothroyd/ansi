...
| Wide screens (`shared/ansi_layout.dart`) | 🟡 | One file reads the viewport — three bands off Forui's own breakpoints, and a 640 measure applied in exactly two places (the tab shell wraps what it owns, the router's page helper wraps every pushed page), held by a structural test that fails on a `MediaQuery.sizeOf`, a `.size` or a `LayoutBuilder` anywhere else under `lib/`. A wide window centres the phone layout and does nothing more with the room: no chrome beside the content, sheets are still sheets, and no view yet uses the width. | [wide-screen.md](./docs/design-docs/wide-screen.md) |
